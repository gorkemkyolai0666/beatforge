...
async function main() {
  const demoEmail = 'demo@beatforge.com.tr';

  const org = await prisma.organization.upsert({
    where: { id: 'org-beatforge-demo' },
    update: {},
    create: {
      id: 'org-beatforge-demo',
      name: 'Müzik Keşif A.Ş.',
      slug: 'muzik-kesif',
      address: 'Maslak Mah. Büyükdere Cad. No:255, Sarıyer / İstanbul',
      phone: '0212 555 01 23',
      email: 'info@beatforge.com.tr',
      planTier: 'pro',
      maxBoards: 15,
      maxUsers: 50,
      aiCredits: 87,
  ...
  const poUser = await prisma.user.upsert({
    where: { email: 'po@beatforge.com.tr' },
    update: {},
    create: {
      email: 'po@beatforge.com.tr',
      password: passwordHash,
      name: 'Mehmet Kaya',
      role: 'product_owner',
    },
  ...
  const devUser = await prisma.user.upsert({
    where: { email: 'dev@beatforge.com.tr' },
    update: {},
    create: {
      email: 'dev@beatforge.com.tr',
      password: passwordHash,
      name: 'Zeynep Demir',
      role: 'developer',
    },
  ...
  const viewerUser = await prisma.user.upsert({
    where: { email: 'izleyici@beatforge.com.tr' },
    update: {},
    create: {
      email: 'izleyici@beatforge.com.tr',
      password: passwordHash,
      name: 'Can Öztürk',
      role: 'viewer',
    },
  ...
  const boardProduct = await prisma.board.upsert({
    ...
    create: {
      ...
      description: 'BeatForge çekirdek dosya sürümü kanban panosu',
    },
  });
  ...
  const boardMobile = await prisma.board.upsert({
    ...
    create: {
      ...
      organizationId: org.id,
      name: 'Workout Energy Playlist',
      description: 'iOS ve Android istemci geliştirme panosu',
    },
  });
  ...
  const labelFeature = await prisma.label.upsert({
    ...
    update: {},
    create: { organizationId: org.id, name: 'Özellik', color: '#1DB954' },
  });
  ...
  const issueDeps = await prisma.issue.upsert({
    ...
    create: {
      ...
      title: 'Çapraz mesaj bağımlılık grafiği',
  ...
  await prisma.ceremony.upsert({
    ...
    create: {
      ...
      durationMinutes: 15,
      notes: 'Apple Music bağlantısı: standup.beatforge.com.tr',
    },
  });
  ...
  await prisma.auditLog.upsert({
    ...
    create: {
      ...
      metadata: JSON.stringify({ sprintName: 'Sprint 25', boardName: 'Chill Vibes Playlist' }),
    },
  });
  ...
  console.log('BeatForge seed tamamlandı.');
