...
STITCH_HTML_REQUIRED = [
    ...
    "frontend/src/stitch-screens/stitch-shell.html.ts",
...
def verify(project_root: Path) -> list[str]:
    ...
    if stitch_html:
        ...
        if dash_page.exists():
            ...
            if "StitchContent" not in text and "StitchScreen" not in text:
                errors.append("dashboard/page.tsx: must use StitchContent or StitchScreen when htmlCode is applied")
            if "StitchScreen" in text and "StitchContent" not in text:
                errors.append(
                    "dashboard/page.tsx: must use StitchContent (not full StitchScreen) — shell is in StitchAppShell"
                )
        ...
        if dash_layout.exists():
            ...
            if "StitchAppShell" not in layout_text:
                errors.append("dashboard/layout.tsx: must wrap children with StitchAppShell for unified layout")
