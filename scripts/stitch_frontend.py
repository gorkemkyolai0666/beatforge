...
def generate_stitch_frontend(project_root: Path) -> list[str]:
    ...
    has_html = stitch_has_html_code(project_root)
    ...
    if use_html and has_html:
        ...
        manifest = project_root / "docs/project/stitch-applied-manifest.json"
        landing_from_stitch = False
        if manifest.exists():
            manifest_data = json.loads(manifest.read_text(encoding="utf-8"))
            landing_from_stitch = any(
                r.endswith("app/page.tsx") for r in manifest_data.get("routes", [])
            )
        if not landing_from_stitch:
            write_landing_page(fe, cfg)
            files.append("frontend/src/app/page.tsx")
            print("  Landing: no Stitch Landing screen — generated token-based fallback page")
    else:
        write_landing_page(fe, cfg)
