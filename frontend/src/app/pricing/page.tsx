...
const tiers = [
  {
    ...
    features: ['3 mesaj', '5 kullanıcı', 'Temel kanban', '1 aktif sprint', 'E-posta desteği'],
  ...
  },
  {
    ...
    highlight: true,
    features: ['Sınırsız mesaj', '25 kullanıcı', 'Velocity-aware sprint planner', 'Cross-board bağımlılık grafiği', 'Burndown & velocity analitiği', 'AI sprint kredileri (50/ay)', 'Öncelikli destek'],
  ...
  },
  {
    ...
    features: ['Sınırsız kullanıcı & mesaj', 'SSO/SAML entegrasyonu', 'KVKK denetim raporları', 'API & webhook erişimi', 'Çoklu workspace', 'Özel hesap yöneticisi', 'White-label portal', 'SLA garantisi'],
    missing: [],
  },
];

const comparison = [
  { feature: 'Kanban panoları', starter: '3', pro: 'Sınırsız', enterprise: 'Sınırsız' },
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span className="font-display text-lg font-extrabold">BeatForge</span>
          </Link>
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm font-medium text-muted-foreground hover:text-foreground">Giriş</Link>
            <Link href="/register" className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-emerald-dark">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground">
          ...
          BeatForge Pro: <span className="font-mono text-emerald-light">₺299/kullanıcı/ay</span>
        </p>
      </section>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {tiers.map((t) => (
            <div key={t.id} className={t.highlight ? 'surface-card-accent p-7' : 'surface-card p-7'}>
              {t.highlight && <span className="mb-3 inline-block rounded-full bg-emerald px-3 py-1 text-xs font-semibold text-primary-foreground">En popüler</span>}
              ...
              <Link href="/register" className={t.highlight
                ? 'mt-7 inline-flex w-full items-center justify-center rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-emerald-dark'
      ...
      <footer className="mx-auto max-w-6xl px-4 py-10 text-center text-sm text-muted-foreground">
        <p>© {new Date().getFullYear()} BeatForge · Tüm paylaşımlar KDV hariçtir.</p>
      </footer>
    </div>
  );
}
