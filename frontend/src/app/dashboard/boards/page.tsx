...
const demoBoards: Board[] = [
  { id: '1', name: 'Dosya Sürümü Geliştirme', description: 'Ana dosya sürümü backlog ve sprint panosu', issueCount: 24, updatedAt: new Date().toISOString() },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold">Parçalar</h1>
          <p className="text-muted-foreground">Kanban panolarınızı yönetin</p>
        </div>
        <div className="flex items-center gap-3">
          {usingDemo && <Badge variant="warning">Demo veri</Badge>}
          <Button><Plus className="mr-2 h-4 w-4" />Yeni Parça</Button>
        </div>
      </div>
      ...
      {!loading && boards.length === 0 && (
        <EmptyState title="Henüz mesaj yok" description="İlk kanban mesajnızı oluşturarak başlayın." action={<Button><Plus className="mr-2 h-4 w-4" />Mesaj Oluştur</Button>} />
      )}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {boards.map((board) => (
            <Link key={board.id} href={`/dashboard/boards/${board.id}`}>
              <Card className="h-full transition-colors hover:border-emerald/40 hover:shadow-emerald-glow">
                <CardHeader>
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald/10 text-emerald-light">
