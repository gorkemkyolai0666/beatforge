...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState('demo@beatforge.com.tr');
      ...
      <Card className="w-full max-w-md border-border/80 bg-card/95 backdrop-blur">
        <CardHeader>
          <div className="mb-2 flex items-center gap-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald text-primary-foreground"><Layers className="h-5 w-5" /></div>
        ...
        <CardContent>
          ...
          <div className="mt-4 rounded-lg border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
            <p className="font-mono">Demo: demo@beatforge.com.tr / demo123456</p>
          </div>
          <p className="mt-4 text-center text-sm text-muted-foreground">
            Hesabınız yok mu? <Link href="/register" className="font-medium text-emerald-light hover:underline">Dosya Sürümü olun</Link>
          </p>
        </CardContent>
      </Card>
    </div>
